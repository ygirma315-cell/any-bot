...
export const OrderScreen: React.FC<OrderScreenProps> = ({
  ...
  if (cart.length === 0) {
    return (
      <div className="max-w-md mx-auto px-4 py-16 flex flex-col items-center justify-center text-center space-y-6 min-h-[60vh] animate-fadeIn">
        ...
        <div className="space-y-1.5 max-w-xs">
          <h2 className="heading-font text-xl font-extrabold text-slate-900">Your cart is empty</h2>
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 space-y-5 pb-28 animate-fadeIn">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="heading-font text-lg sm:text-xl font-bold text-slate-900">YOUR ORDER</h2>
          <p className="text-xs text-slate-500">Review selected services &amp; specify your delivery email</p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
        ...
        <div className="lg:col-span-7 space-y-3.5">
          {cart.map(({ product, quantity }) => {
            ...
            return (
              <div
                key={product.id}
                className="p-4 bg-white/95 backdrop-blur-md rounded-2xl border border-slate-200/80 shadow-xs space-y-3 transition-all hover:shadow-md"
              >
                ...
                <div className="flex items-center justify-between gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <h3 className="heading-font text-xs sm:text-sm font-bold text-slate-900 truncate">
                      {product.name}
                    </h3>
        ...
        {/* Right Column: Sticky Checkout Sidebar (5 cols on lg) */}
        <div className="lg:col-span-5 space-y-4 lg:sticky lg:top-4">
          ...
          <div className="p-4 bg-white/95 backdrop-blur-md rounded-2xl border border-orange-200/80 shadow-sm space-y-2.5">
            ...
            <div className="p-2.5 bg-orange-50/70 rounded-xl border border-orange-100 text-[11px] text-orange-950 space-y-1">
              <p className="font-bold flex items-center gap-1">
                <Info className="w-3.5 h-3.5 text-[#FF6B00] shrink-0" /> Delivery Notes:
              </p>
              <p className="text-slate-700 leading-snug">
                • Digital product credentials will be delivered directly to this email upon payment confirmation.<br />
                • <strong>1 Product per 1 Email:</strong> Double check your spelling to avoid delivery delays.
              </p>
            </div>
          </div>

          {/* Summary Calculation Box */}
          <div className="p-4 bg-white/95 backdrop-blur-md rounded-2xl border border-slate-200/80 shadow-xs space-y-2.5">
            ...
            <div className="pt-2.5 border-t border-slate-100 flex items-center justify-between text-sm">
              ...
              <span className="heading-font text-xl font-extrabold text-[#FF6B00]">${total.toFixed(2)}</span>
            </div>
          </div>
          ...
          <button
            ...
            className="w-full py-3.5 px-5 rounded-2xl text-xs font-extrabold shadow-md hover:shadow-lg flex items-center justify-center gap-2 bg-[#FF6B00] hover:bg-[#E66000] text-white active:scale-95 transition-all"
          >
